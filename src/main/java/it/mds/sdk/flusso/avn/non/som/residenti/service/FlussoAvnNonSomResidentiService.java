...
@Slf4j
@Component("flussoAvnNonSomResService")
public class FlussoAvnNonSomResidentiService {
    private static final String MDS_RESP_OK = "IF00";
    private static final String TAG_FLUSSO_XML = "</vaccinazioniNonEffettuate><vaccinazioniNonEffettuate CodiceRegione=";
    private ParserRegole parserRegole;
    private ParserTracciato parserTracciato;
    private ValidationEngine validationEngine;
    private TracciatoSplitter<RecordDtoAvnNonSomResidenti> tracciatoSplitter;
    private ConnettoreMds connettoreMds;
    private ConfigurazioneFlussoAvnNonSomResidenti conf;
    private Crittografia crittografia;
    private final String NOME_FILE_XSD_AVN = "VNX.xsd";
    ...
    @Autowired
    public FlussoAvnNonSomResidentiService(@Qualifier("parserRegoleAvnNonSomResidenti") final ParserRegole parserRegole,
                                           ...
                                           @Qualifier("tracciatoSplitterAvnNonSomRes") final TracciatoSplitter<RecordDtoAvnNonSomResidenti> tracciatoSplitter,
                                           @Qualifier("connettoreMds") final ConnettoreMds connettoreMds,
                                           @Qualifier("configurazioneFlussoAvnNonSommResidenti") final ConfigurazioneFlussoAvnNonSomResidenti conf,
    ...
    public void validazioneBlocchi(int dimensioneBlocco, String nomeFile, RegoleFlusso regoleFlusso, String idRun,
        ...
        VaccinazioniNonEffettuate vaccinazioniNonEffettuate = null;
        File file = new File(conf.getFlusso().getPercorso() + nomeFile);
        String nomeFileXml = conf.getXmlOutput().getPercorso() + "SDK_AVN_VNX_" + periodoRiferimento + "_" + idRun + ".xml";
        ...
        do {
            ...
            List<RecordDtoAvnNonSomResidenti> recordConv =
                    bloccoValidazione.getRecordList().stream().map(k -> (RecordDtoAvnNonSomResidenti) k).collect(Collectors.toList());
            ...
            if (!recordConv.isEmpty()) {
                ...
                vaccinazioniNonEffettuate = ((TracciatoSplitterImpl) tracciatoSplitter).creaVaccinazioniNonEffettuate(recordConv, vaccinazioniNonEffettuate);
            } else {
            ...
            if (isWriteOk) {
                ...
                try {
                    gestoreFile.scriviDtoFragment(vaccinazioniNonEffettuate, nomeFileXmlTmp, urlXsd);
                ...
                vaccinazioniNonEffettuate = null;
                System.gc();
            }
